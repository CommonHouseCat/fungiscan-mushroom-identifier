...
Original file is located at
    https://colab.research.google.com/drive/17z4vmYDV7l1V6kJ6mgm6_1Br1jWnlVCt
"""
...
optimizer = optim.AdamW([
    {"params": model.module.vit.parameters(), "lr": 3e-5},
...
plt.savefig("Vision_Transformer_confusion_matrix.png")
...
plt.savefig("Vision_Transformer_loss.png")
...
plt.savefig("Vision_Transformer_acc.png")
...
def extract_vit_features(model, loader, device, max_samples=1500):
    model.eval()
    feats, labels = [], []
    count = 0
    with torch.no_grad():
        for imgs, lbls in loader:
            ...
            if hasattr(model, "vit"):
                outputs = model.vit(imgs)
                if isinstance(outputs, (tuple, list)):
                    feats_batch = outputs[0][:, 0, :]
                elif hasattr(outputs, "last_hidden_state"):
                    feats_batch = outputs.last_hidden_state[:, 0, :]
                else:
                    feats_batch = outputs
            else:
                outputs = model(imgs)
                if outputs.ndim == 2:
                    ...
                    feats_batch = outputs.mean(dim=[2, 3])
...
features, labels = extract_vit_features(model, train_loader, device)
...
plt.title("t-SNE Visualization of Vision Transformer Latent Features (CLS Token)")
plt.savefig("Vision_Transformer_tsne_visualization.png")
plt.show()

# # ================== SAVE MODEL FOR DEPLOYMENT ==================
# save_path = os.path.join(OUTPUT_DIR, "vit.pth")
